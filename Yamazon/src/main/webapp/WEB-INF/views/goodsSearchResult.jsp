	...
	<div class="text-center" style="margin: 0px 0px 20px 0px">
		<table class="table table-hover"
			...
			<c:forEach var="goods" items="${goods}">
				<tr>
					<td>${fn:escapeXml(goods.id)}</td>
					<td><img src="${fn:escapeXml(goods.image)}" width="100" height="120"></td>
					<td style="width: 120px; height: auto;">${fn:escapeXml(goods.name)}</td>
					<td>${fn:escapeXml(goods.category)}</td>
					<td>${fn:escapeXml(goods.price)}</td>
					<td>${fn:escapeXml(goods.taxPrice)}</td>
					<td>${fn:escapeXml(goods.stock)}</td>
					<td style="width: 350px; height: auto;">${fn:escapeXml(goods.explain)}</td>
					<td><button type="submit" value="${fn:escapeXml(goods.number)}"
							onclick="location.href='goodsUpdateConfirm'">更新</button></td>
					<td><button type="submit" value="${fn:escapeXml(goods.number)}" onclick="MoveCheck();">削除</button></td>
				</tr>
			</c:forEach>
			</tbody>
		</table>
